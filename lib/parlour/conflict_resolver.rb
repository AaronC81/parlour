...
module Parlour
  ...
  class ConflictResolver
    ...
    def resolve_conflicts(namespace, &resolver)
      Debugging.debug_puts(self, "=== Begin #{namespace.name}")
      ...
      grouped_by_name_children.each do |name, children|
        Debugging.debug_puts(self, "Checking #{name}...")

        if children.length > 1
          Debugging.debug_puts(self, "Possible conflict between #{children.length} objects")
          ...
          if children.length == 2 &&
            ...
            Debugging.debug_puts(self, "One is an instance method and one is a class method; no resolution required")
            next
          end
          ...
          if children.length == 2 &&
            ...
            Debugging.debug_puts(self, "One is an instance attribute and one is a class attribute; no resolution required")
            next
          end

          # Special case: are they all clearly equal? If so, remove all but one
          if all_eql?(children)
            Debugging.debug_puts(self, "All children are identical")
          ...
          unless children_type
            Debugging.debug_puts(self, "Children are different types; requesting manual resolution")
          ...
          if T.must(first).mergeable?(T.must(rest))
            Debugging.debug_puts(self, "Children are all mergeable; resolving automatically")
          ...
          Debugging.debug_puts(self, "Unable to resolve automatically; requesting manual resolution")
          choice = resolver.call("Can't automatically resolve", children)
          namespace.children << choice if choice
        else
          Debugging.debug_puts(self, "No conflicts")
        end
      end

      Debugging.debug_puts(self, "Recursing to children...")
      ...
      Debugging.debug_puts(self, "=== End #{namespace.name}")
    end
